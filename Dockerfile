...
# Blaze
COPY blaze-3.6.tar.gz /
COPY build-engine-Desktop_Qt_5_12_11_GCC_64bit-Release /
# Install Blaze and Atari
RUN tar -xvzf blaze-3.6.tar.gz &&\
	cd blaze-3.6 &&\
	cmake -DCMAKE_INSTALL_PREFIX=/usr/local/ &&\
	sudo make install &&\
	mkdir AtariAra &&\
	cd AtariAra &&\
	git clone https://github.com/chooga/CrazyAra.git --recursive . &&\
	cd engine &&\
	mkdir build &&\
	cd build &&\
